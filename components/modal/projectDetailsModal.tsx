...
export interface ProjectData {
    ...
    details: {
        servicesProvided: string[], hourlyRate: string,
        monthlyHours: string,
        packageLevel: string, postsPerWeek: string, platforms: string[],
        projectType: string, currentPhase: string, technologies: string[],
        managementAreas: string[]
    };
};


export default function ProjectDetailModal({ onClose, data}: { onClose: () => void, data:ProjectData | null}) {
    if(!data){
        return null;
    }
    ...
    const iconMap: Record<string, { bg: string; text: string; icon: string }> = {
        va: { bg: 'bg-blue-100', text: 'text-blue-500', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
        obm: { bg: 'bg-indigo-100', text: 'text-indigo-500', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
        smm: { bg: 'bg-purple-100', text: 'text-purple-500', icon: 'M7 11.5V14m0-2.5v-6a1.5 1.5 0 113 0m-3 6a1.5 1.5 0 00-3 0v2a7.5 7.5 0 0015 0v-5a1.5 1.5 0 00-3 0m-6-3V11m0-5.5v-1a1.5 1.5 0 013 0v1m0 0V11' },
        wds: { bg: 'bg-rose-100', text: 'text-rose-500', icon: 'M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4' }
    };
        ...
        return (
            <div className="space-y-4">
                <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="text-gray-900 font-medium">{data.progress.used}/{data.progress.total} hrs</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-1.5">
                    <div className={`bg-${data.type === 'va' ? 'blue' : 'indigo'}-500 h-1.5 rounded-full`}
                        style={{ width: `${data.progress.percent}%` }}></div>
                </div>
                <div className="grid grid-cols-3 gap-2 mt-4">
                    ...
                    <div className="text-center p-2 bg-gray-50 rounded-lg">
                        ...
                        <div className="text-sm font-medium text-gray-900">${data.progress.total - data.progress.used} hrs</div>
                    </div>
                </div>
            </div>
        );
    ...
    const getMilestonesTemplate = (data: ProjectData) => {
        ...
        return (
            (milestones[data.type] || []).map((milestone, index) => (
                <div key={index} className={`text-center p-2 ${index < 2 ? 'bg-green-50' : index === 2 ? 'bg-blue-50' : 'bg-gray-50'} rounded-lg`}>
                    <div className={`text-xs ${index < 2 ? 'text-green-600' : index === 2 ? 'text-blue-600' : 'text-gray-500'}`}>
        ...
        return (
            <div className="space-y-4">
                <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="text-gray-900 font-medium">{data.progress.percent}%</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-1.5">
                    <div className={`bg-${data.type === 'smm' ? 'purple' : 'rose'}-500 h-1.5 rounded-full`}
                        style={{ width: `${data.progress.percent}%` }}></div>
                </div>
            ...
            return (
                type == "va" ? (
                    <>
                        ...
                        <div className="flex items-center justify-between text-sm">
                            <span className="text-gray-500">Monthly Hours</span>
                            <span className="text-gray-900">{details.monthlyHours || 'N/A'}</span>
                        </div>
                        <div className="mt-3">
                            <span className="text-xs font-medium text-gray-500">Services Provided</span>
                    ...
                    <>
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="text-gray-900">{details.packageLevel || 'N/A'}</span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                            <span className="text-gray-500">Posts per Week</span>
                            <span className="text-gray-900">{details.postsPerWeek || 'N/A'}</span>
                        </div>
                        <div className="mt-3">
                    ...
                    <>
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="text-gray-900">{details.projectType || 'N/A'}</span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                            <span className="text-gray-500">Current Phase</span>
                            <span className="text-gray-900">{details.currentPhase || 'N/A'}</span>
                        </div>
                        <div className="mt-3">
                            <span className="text-xs font-medium text-gray-500">Technologies</span>
                            <div className="mt-2 flex flex-wrap gap-2">
                                {(details.technologies || []).map((tech,index) => (
                                    <span key={index} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-50 text-rose-700">
                                        {tech}
                                    </span>
                                ))}
                            </div>
                    ...
                    <>
                        ...
                        <div className="mt-3">
                            <span className="text-xs font-medium text-gray-500">Management Areas</span>
                            <div className="mt-2 flex flex-wrap gap-2">
            ...
            <>
                ...
                <div className="relative pl-10 pb-6  border-gray-500/50 border-l-2 ">
                    ...
                    <p className="mt-2 text-sm text-gray-600 bg-gray-50 rounded-lg p-3">
                        Updated the content calendar for next week. Please review when you have a chance.
                    </p>
        ...
        <div id="projectDetailsModal"
            ...
            <div className="relative flex flex-col max-h-[90vh]">
                ...
                <div className="sticky top-0 z-20 bg-white px-6 py-4 border-b border-gray-100">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div id="projectTypeIcon" className={`w-10 h-10 rounded-lg flex items-center justify-center ${iconMap[type].bg}`}>
                                {/* <!-- Icon will be inserted dynamically --> */}
                                <svg className={`w-5 h-5 ${iconMap[type].text}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d={`${iconMap[type].icon}`} />
                                </svg>
                            </div>
                            <div>
                                <h2 id="projectTitle" className="text-xl font-semibold text-gray-900">{projectTitle}</h2>
                                <p id="clientName" className="text-sm text-gray-500">{clientName}</p>
                            </div>
                        </div>
                        <button type="button"
                            onClick={onClose}
                        ...
                        <div className="flex items-center justify-between bg-gray-50 rounded-xl p-4">
                            <div className="flex items-center gap-3">
                                ...
                                <span id="projectDates" className="text-sm text-gray-500">
                                    ...
                                    {
                                        dates.due ? "Due" + dates.due : dates.renewal ? "Renews" + dates.renewal :
                                            "Started " + dates.start
                                    }
                                </span>
                            </div>
                            <button className="text-sm text-brand-500 hover:text-brand-600 font-medium">Edit
                                Project</button>
                        </div>
                                    ...
                                    {/* <!-- Team members will be inserted dynamically --> */}
                                    {
                                        ...
                                        data.teamMembers.map((member, index) => (
                                            <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                                                <img src={member.image} alt={member.name} className="w-8 h-8 rounded-lg object-cover" />
                                                <div>
                ...
                <div className="sticky bottom-0 bg-white border-t border-gray-100 px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <button className="text-sm text-gray-500 hover:text-gray-700 font-medium">View Time
                                Logs</button>
                            <button className="text-sm text-gray-500 hover:text-gray-700 font-medium">View Files</button>
                        </div>
                        <div className="flex items-center gap-3">
                            <button className="px-4 py-2 text-sm font-medium text-red-600 hover:text-red-700">Archive
                                Project</button>
                            <button
                                className="px-4 py-2 text-sm font-medium text-white bg-brand-500 rounded-lg hover:bg-brand-600">Start
                                Timer</button>
                        </div>
                    </div>
                </div>
