...
import { TypeProject, TypeTask, TypeUser } from '@/lib/types';
...
export default function Projects() {
  ...
  useEffect(() => {
    ...
    const fetchClients = async () => {
      const response = await fetch('http://localhost:5173/admin/clients', {
        method: 'GET',
        headers: {
    ...
    const fetchUsers = async () => {
      const res = await fetch('http://localhost:5173/admin/team', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          authorization: `Bearer ${token}`
