      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Manage your financial operations</p>
          </div>
            ...
            <div key={index} className="bg-white rounded-xl border border-gray-100 p-6">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${
                  metric.trendUp ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'
                }`}>
                  {metric.trend}
                </span>
              </div>
              <h3 className="text-2xl font-semibold text-gray-900 mt-4">{metric.value}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100">
            <div className="p-6 border-b border-gray-100">
              <h2 className="text-lg font-medium text-gray-900">Package Utilization</h2>
            </div>
            <div className="p-6 space-y-4">
              {packageUtilization.map((pkg, index) => (
                <div key={index} className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-900">{pkg.type}</span>
                    <span className="text-sm text-gray-500">{pkg.total} clients</span>
                  </div>
                  <div className="relative">
                    <div className="h-2 bg-gray-100 rounded-full">
                      <div 
                        className="h-2 bg-brand-500 rounded-full"
                        style={{ width: `${(pkg.hours.used / pkg.hours.total) * 100}%` }}
                      ></div>
                    </div>
                ...
                <div key={index} className="p-4 hover:bg-gray-50">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{activity.package}</p>
                    </div>
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium bg-${activity.badgeColor}-50 text-${activity.badgeColor}-600`}>
                      {activity.status}
                    </span>
                  </div>
